...
**Yêu cầu:** Python 3.11 trở lên. Cần **Gemini API key** để chạy
`domain_assistant.py` với provider mặc định của submission (Part 3 — sinh 20
actual answers từ RAG thật). OpenAI vẫn được hỗ trợ như provider fallback; phần
code core (`template.py`, Part 1–2) không cần API key.
